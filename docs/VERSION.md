...
- 投稿メニューにネタページ（/blog-ideas）を追加し、ブログ下書きから SideBusiness / Agri タブを移した
- ネタを Google スプレッドシート連携から D1 保存＋CSV アップロードに切り替えた
- ネタに税務・DX・業務プロセス改革・M＆A・行政手続きタブを追加した
- 各ネタのブログ / X 転用先（媒体）を記録できるようにした

## 0.0.65 (2026-09-21)

- X投稿スケジュールに投稿先タブ「スタジオフーズ」を追加した
- デプロイ時の Mermaid CDN import の型エラーを修正した
- X投稿スケジュール一覧のステータスをボタン表示にし、ワンクリックで変更できるようにした

## 0.0.64 (2026-09-20)

- 会計マニュアルNo.28〜34の詳細本文を追加（貸倒引当金・賞与退職給付引当金・少額減価償却・減価償却・特別償却）
- X投稿先に MIERU会計（mieru）を追加

## 0.0.63 (2026-09-20)

- トップナビに「投稿」を追加し、ブログ下書きとX投稿スケジュールをまとめた
- X投稿スケジュールのステータス色を、下書き・承認済・予約・完了・失敗で明確に分けた
- 未公開ブログ下書きに複数AIのコメントを保存・確認できるようにした
- コメントに未対応／対応済のステータスを付けられるようにした
- ブログ下書き一覧で使用予定画像（TOP／図表）を確認できるようにした
- X投稿スケジュールの投稿先タブ表示名を「農業日誌漫画」に変更した

## 0.0.62 (2026-09-19)

- ブログ下書きに SideBusiness / Agri のネタシートを同じ画面で参照できるようにした
- Google Tasks 同期を全リスト取得に対応した（新規の台帳タスクはリスト ai-org へ）
- 会計マニュアルNo.12「不良債権を調べる②（個別評価貸倒引当金）」の詳細本文を追加
- 会計マニュアルNo.14「減価償却の仕組みを理解する」の詳細本文を追加
...
- トップナビに「情報」を追加し、掲示板と会計マニュアルをまとめた
- 会計マニュアルに法人税決算・申告の目次（No.1〜62）を載せた

## 0.0.59 (2026-09-18)

- トップナビの従業員・組織ルール・自動化一覧を「組織」メニューにまとめた

## 0.0.58 (2026-09-18)

- X投稿スケジュールを投稿先アカウントのタブで分けられるようにした

## 0.0.57 (2026-09-17)

- ブログ下書きで TOP 画像と図表・グラフをアップロードできるようにした
- 確認画面の Markdown 本文を読みやすく表示する

## 0.0.56 (2026-09-17)

- 農業日誌アプリLPのブログ下書きを投稿管理スプレッドシートと双方向同期する

## 0.0.55 (2026-09-14)

- スタジオフーズHPのブログ下書きを投稿管理スプレッドシートと双方向同期する

## 0.0.54 (2026-09-14)

- 上部ナビに加えて、左から出し入れできるドロワーメニューを追加

## 0.0.53 (2026-09-13)

- 掲示板ページを追加し、指定スプレッドシートへのリンクを上部に置いた
- Google Tasks の直近同期結果を業務台帳に表示する
- 掲示板に「ブログネタ」スプレッドシートを追加
- 掲示板に最新の変更履歴（VERSION.md から取得）を表示
- 掲示板に「農業_ブログ記事_投稿管理」スプレッドシートを追加
...
- AI生成のX投稿文を文字数制限（ウェイト280）以内に収める
...
- 画面読み込み失敗時の error 境界を追加し、一覧更新失敗時はフルリロードで復旧
...
- Apps シートを閲覧用チップ表示にし、編集はダイアログで行うよう変更

## 0.0.35 (2026-08-22)

- 業務台帳ガントの日付見出し行をスクロール時に固定
...
- X投稿スケジュール一覧を予約日時の降順で表示
- ステータス変更・再投稿時に last_error をクリアし、失敗時のみエラーを表示

## 0.0.29 (2026-08-14)

- X投稿の画像解析を Gemini 3.6 Flash に統一し、テーマ分析と3パターンの投稿案を生成
- 体裁名（「漫画」など）がタイトルになる問題を修正し、分析結果をフォームに表示
